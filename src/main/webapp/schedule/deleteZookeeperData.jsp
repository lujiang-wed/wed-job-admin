...
<script>
function queryPath(){
    location.href= "deleteZookeeperData.jsp?action=query&path=" + document.all("path").value;
}
function deletePath(){	
    location.href="deleteZookeeperData.jsp?action=delete&path=" + document.all("path").value;
}
function openDelete(){	
    document.all("deleteButton").disabled = document.all("openDeleteButton").checked === false;
}
</script>
